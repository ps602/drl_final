...
#TODO: add gumbel softmax, onehot from logits
"""
initializers
"""
...
def onehot_from_logits(logits):
    ...
    rand_acs = tf.Variable(tf.eye(logits.shape[1])[[np.random.choice(range(logits.shape[0]))]])
    return tf.stack([argmax_acs[i] if r > eps else rand_acs[i] for i, r in
...
class ReplayBuffer(TransBuffer):#TODO: check if enough
    ...
    def sample_transition(self):
        ...
        next_state_batch = np.asarray([data[2] for data in minibatch])
