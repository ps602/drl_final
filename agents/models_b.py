...
from agents.utils import *  # all the functions under agents' utils.py
from agents.policies import *  # all the functions under agents' policy.py
import logging
import multiprocessing as mp  # multiple threads for faster execution
...
class A2C:
    def __init__(self, n_s, n_a, total_step, model_config, seed=0, n_f=None):
        # load parameters
        self.name = 'a2c'  # declaring name as a2c
        self.n_agent = 1  # declaring number of agents as 1
        # init reward norm/clip
        self.reward_clip = model_config.getfloat('reward_clip')  # declaring reward_clip from model_config's variable reward_clip
        self.reward_norm = model_config.getfloat('reward_norm')  # declaring reward_norm from model_config's variable reward_norm
        self.n_s = n_s  # declaring n_s (number of states??)
        self.n_a = n_a  # number of actions??
        self.n_step = model_config.getint('batch_size')  # obtaining n_step info from batch_size in model_config
        # init tf
        #tf.reset_default_graph()
        tf.set_random_seed(seed)  # sets a random seed
        config = tf.ConfigProto(allow_soft_placement=True)
        self.sess = tf.Session(config=config)
        self.policy = self._init_policy(n_s, n_a, n_f, model_config)  # calling init_policy function
        self.saver = tf.train.Saver(max_to_keep=5)
        if total_step:
            # training
            self.total_step = total_step  # if total_step is non zero setting total_step
            self._init_scheduler(model_config)  # initialising scheduler
            self._init_train(model_config)  # initialising train
        self.sess.run(tf.global_variables_initializer())

    def _init_policy(self, n_s, n_a, n_w, n_f, model_config, agent_name=None):
        n_fw = model_config.getint('num_fw')  # number of wave states
        n_ft = model_config.getint('num_ft')  # number of wait states
        n_lstm = model_config.getint('num_lstm')  # number of lstms
        if self.name == 'ma2c':  # checking if agent is ma2c
            n_fp = model_config.getint('num_fp')  # ?? something called fp is only for ma2c
            policy = FPLstmACPolicy(n_s, n_a, n_w, n_f, self.n_step, n_fc_wave=n_fw,
                                    n_fc_wait=n_ft, n_fc_fp=n_fp, n_lstm=n_lstm, name=agent_name)  # creating an instance for the FPLstmACPolicy class in policy.py
        else:
            policy = LstmACPolicy(n_s, n_a, n_w, self.n_step, n_fc_wave=n_fw,  # creating an instance for the LstmACPolicy class in policy.py for non ma2c
                                  n_fc_wait=n_ft, n_lstm=n_lstm, name=agent_name)
        return policy

    def _init_scheduler(self, model_config):
        lr_init = model_config.getfloat('lr_init')  # declaring variable lr_init from model_config
        lr_decay = model_config.get('lr_decay')  # declaring variable lr_decay from model_config
        beta_init = model_config.getfloat('entropy_coef_init')  # declaring variable beta_init from model_config's entropy_coef_init
        beta_decay = model_config.get('entropy_decay')  # declaring variable beta_decay from model_config's entropy decay
        if lr_decay == 'constant':  # checking if lr_decay is constant
            self.lr_scheduler = Scheduler(lr_init, decay=lr_decay)  # creating object for scheduler class and calling it lr_scheduler
        else:  # if lr_decay is not constant
            lr_min = model_config.getfloat('LR_MIN')  # declaring lr_min as model_config's LR_MIN
            self.lr_scheduler = Scheduler(lr_init, lr_min, self.total_step, decay=lr_decay)   # declaring an object of the Scheduler class
        if beta_decay == 'constant':  # if beta_decay is constant
            self.beta_scheduler = Scheduler(beta_init, decay=beta_decay)  # creating an object of the Scheduler class
        else:  # if beta_decay is not constant
            beta_min = model_config.getfloat('ENTROPY_COEF_MIN')  # declaring beta_min as model_config's entropy_coef_min
            beta_ratio = model_config.getfloat('ENTROPY_RATIO')  # declaring beta_ratio as model_config's entropy_ratio
            self.beta_scheduler = Scheduler(beta_init, beta_min, self.total_step * beta_ratio,
                                            decay=beta_decay)  # declaring object of scheduler class

    def _init_train(self, model_config):  # init_train function
        # init loss
        v_coef = model_config.getfloat('value_coef')  # declaring v_coef as value_coef
        max_grad_norm = model_config.getfloat('max_grad_norm')  # declaring max_grad_norm as max_grad_norm
        alpha = model_config.getfloat('rmsp_alpha')  # declaring alpha as rmsp_alpha
        epsilon = model_config.getfloat('rmsp_epsilon')  # declaring epsilon as rmsp_epsilon
